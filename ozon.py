...
DRIVER = webdriver.Chrome(options=CHROME_OPTIONS,executable_path='/Users/aleksandrnikolich/Downloads/chromedriver 2')
TITLE_HTML = 'a2g0'
PRICE_HTML = 'b5v6'
...
def get_items_on_page(driver):
    page_scroll(driver)
    length = driver.find_elements_by_class_name('a0c6 a0c9 a0c8')
    return(len(length))
...
def get_pages_count(driver, url):
    ...
    items_number = all_items_number_text[0].replace(
        '\u2009', '').split(' ')
    print(get_items_on_page(driver))
