...
class Info(Frame):
    def __init__(self, master=None):
        ...
        self.t = Label(self, text="ход желтых", font=police)
...
class Terrain(Canvas):
    ...
    def detCol(self, event):
        if self.perm:
            ...
            if self.joueur == 1:
                self.joueur = 2
                info.t.config(text="ход красных")
                self.coul = "red"

            elif self.joueur == 2:
                self.joueur = 1
                info.t.config(text="ход желтых")
                self.coul = "yellow"
        ...
        while (i < len(self.p)):
            ...
            while (j < 3):
                if (self.p[i][j].coul == self.p[i][j + 1].coul == self.p[i][j + 2].coul == self.p[i][
                    j + 3].coul == "red"):
                    info.t.config(text="победили красные")
                    self.perm = False
                    break
                elif (self.p[i][j].coul == self.p[i][j + 1].coul == self.p[i][j + 2].coul == self.p[i][
                    j + 3].coul == "yellow"):
                    info.t.config(text="победили желтые")
                    self.perm = False
                    break
                j += 1
            i += 1
...
Button(root, text="Сброс", command=rein).grid(row=2, column=0, pady=30)
